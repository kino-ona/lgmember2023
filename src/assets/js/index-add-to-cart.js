...
$(document).ready(function () {
  ...
  _salesModelCode =
    typeof _dl.products === "undefined" ||
    typeof _dl.products[0] === "undefined"
      ? ""
  ...
  var OBS_Y = {
    changeCartUrl: function (cartID) {
      if (cartID) {
        var url = atcMembersWeek.$nav
          .find(".for-desktop .right-btm .icons .cart a")
          .attr("href");
        if (url) {
          if (url.indexOf("?") != -1) {
          ...
          atcMembersWeek.$nav
            .find(
              ".for-mobile .right .cart a, .for-desktop .right-btm .icons .cart a"
            )
            .attr("href", url);
        }
      }
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (datelayer_use_flag == "Y") {
            OBS_Y.addProductToCart(
              modelId,
              1,
              atcMembersWeek.cartID,
              true,
              addToCartInfo
            );
          } else {
    ...
    addProductToCart: function (model, qty, cartid, ga360, _obj, pinCode) {
      ...
      if (obsReferrerFlag == "Y") {
        cartData = JSON.stringify({
          query:
            'mutation { addModelToCart(input: { cartItems: [ { data: { sku: "' +
            modelSplit[0] +
            '", qty: ' +
            qty +
            '}, package_item_id: "' +
            ...
            '" } ],referrerDomain: "' +
            refererDomain +
            '" }) {redirectUrl cart { items { product { sku } qty}}}}',
          variables: null,
          operationName: null,
        });
      } else if (
        atcMembersWeek.store == "sa" ||
        atcMembersWeek.store == "sa_en"
      ) {
        cartData = JSON.stringify({
          query:
            'mutation { addModelToCart(input: { cart_id: "' +
            cartid +
            '", cartItems: [ { data: { sku: "' +
            modelSplit[0] +
            '", qty: ' +
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.errors) {
            if (
              data.errors[0].message.indexOf("Could not find a cart with ID") !=
                -1 ||
              ...
              if ($("#addToCartError").length > 0)
                $("#addToCartError").remove();
              var html =
                '<div class="modal modal-simple fade" id="addToCartError" tabindex="-1" role="dialog" data-backdrop="false" aria-label="Fail to add Cart" aria-describedby="addToCartFailDialog"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-body"><div class="simple-content-box"><div class="content-paragraph" id="addToCartFailDialog">' +
                errorMsg +
                '</div></div></div><div class="modal-footer"><button type="button" class="btn btn-primary" data-dismiss="modal">' +
                atcMembersWeek.successBtnClose +
                "</button></div></div></div></div>"; /* LGEGMC-243 20200521 modify */
            ...
            var len = 0;
            if (
              data.data.addModelToCart &&
              data.data.addModelToCart.cart &&
              data.data.addModelToCart.cart.items
            ) {
              items = data.data.addModelToCart.cart.items;
              len = items.length;
              if (len > 0) {
                var product =
                  data.data.addModelToCart.cart.items[len - 1].product.sku;
                if (typeof OBS.addCookieValue == "function")
                  OBS.addCookieValue("CART", product);
              }
            }
            if (
              data.data.addExtendedWarrantyToCart &&
              data.data.addExtendedWarrantyToCart.cart &&
              data.data.addExtendedWarrantyToCart.cart.items
            ) {
              items = data.data.addExtendedWarrantyToCart.cart.items;
              len = items.length;
              if (len > 0) {
                var product =
                  data.data.addExtendedWarrantyToCart.cart.items[len - 1]
                    .product.sku;
                if (typeof OBS.addCookieValue == "function")
                  OBS.addCookieValue("CART", product);
              }
            ...
            if (ga360) {
              ...
              if (typeof ga360Init.name != "undefined") {
                productsObj.variant =
                  ga360Init.name.indexOf(".") == -1
                    ? null
                    : ga360Init.name.substring(
                        ga360Init.name.indexOf(".") + 1,
                        ga360Init.name.length
                      );
              }
              if (len > 0) {
            ...
            if (
              data.data.addModelToCart &&
              !data.data.addModelToCart.redirectUrl
            ) {
              // GNB cart number +
              var cnt = 0;
              if (len > 0) {
              ...
              if (
                atcMembersWeek.successHTML == null &&
                successMsg &&
                atcMembersWeek.successBtnAlert &&
                successMsg &&
                atcMembersWeek.successBtnClose
              ) {
                if (
                  ePrivacyCookies.bannerType == "N" ||
                  ePrivacyCookies.bannerType == "I"
                ) {
                  atcMembersWeek.successHTML =
                    '<div class="modal modal-simple fade" id="addToCartSuccess" tabindex="-1" role="dialog" data-backdrop="false" aria-label="Add to Cart" aria-describedby="addToCartSuccessDialog"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-body"><div class="simple-content-box"><div class="content-paragraph" id="addToCartSuccessDialog">' +
                    successMsg +
                    '</div></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-dismiss="modal">' +
                    atcMembersWeek.successBtnClose +
                    '</button><a href="' +
                    xssfilter(
                      atcMembersWeek.$nav.find(".cart a").attr("href")
                    ) +
                    '" class="btn btn-primary">' +
                    atcMembersWeek.successBtnAlert +
                    "</a></div></div></div></div>"; /* LGEGMC-243 20200521 modify, LGEGMC-2485 */
                } else {
                  atcMembersWeek.successHTML =
                    '<div class="modal modal-simple fade" id="addToCartSuccess" tabindex="-1" role="dialog" data-backdrop="false" aria-label="Add to Cart" aria-describedby="addToCartSuccessDialog"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-body"><div class="simple-content-box"><div class="content-paragraph" id="addToCartSuccessDialog">' +
                    successMsg +
                    ...
                    '" class="btn btn-primary">' +
                    atcMembersWeek.successBtnAlert +
                    '</a></div><div class="simple-content-box"><div class="notice ' +
                    noticeIconClass +
                    '" style="display:inline-block;">' +
                    atcMembersWeek.cookieNotice +
                    "</div></div></div></div></div>"; // LGEGMC-2485
                }
              }

              if (atcMembersWeek.successHTML != null) {
                if ($("#addToCartSuccess").length > 0)
                  $("#addToCartSuccess").remove();
              ...
              if (
                ...
                if (
                  ...
                  atcMembersWeek.successHTML =
                    ...
                    '" class="btn btn-primary">' +
                    atcMembersWeek.successBtnAlert +
                    "</a></div></div></div></div>"; /* LGEGMC-243 20200521 modify */
                } else {
                  atcMembersWeek.successHTML =
                    '<div class="modal modal-simple fade" id="addToCartSuccess" tabindex="-1" role="dialog" data-backdrop="false" aria-label="Add to Cart" aria-describedby="addToCartSuccessDialog"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-body"><div class="simple-content-box"><div class="content-paragraph" id="addToCartSuccessDialog">' +
                    successMsg +
              ...
              if (atcMembersWeek.successHTML != null) {
                ...
                $("body")
                  .find(".support-wrap")
                  .first()
                  .append(atcMembersWeek.successHTML);
                ...
                $("#addToCartSuccess")
                  ...
                  .on("click.close", function () {
                    ...
                    var wtyAddToCartCloseUrl = $(".extended-warranty")
                      .find("#wtyAddToCartCloseUrl")
                      .val();
            ...
            if (data.errors) {
              if (
                ...
                (data.errors[0].message.indexOf(
                  ...
                  (atcMembersWeek.$nav.data("store") == "sa" ||
                    atcMembersWeek.$nav.data("store") == "sa_en"))
              ) {
            ...
            if (
              ...
              if (
                ...
                atcMembersWeek.successHTML =
                  '<div class="modal modal-simple fade" id="addToCartSuccess" tabindex="-1" role="dialog" data-backdrop="false" aria-label="Add to Cart" aria-describedby="addToCartSuccessDialog"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-body"><div class="simple-content-box"><div class="content-paragraph" id="addToCartSuccessDialog">' +
                  successMsg +
                  '</div></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary btn-block" data-dismiss="modal">' +
                  atcMembersWeek.successBtnClose +
                  '</button><a href="' +
                  xssfilter(atcMembersWeek.$nav.find(".cart a").attr("href")) +
  ...
  reStockAlert = {
    ...
    autoValidationForm: function () {
      ...
      $("#reStockAlertForm").on({
        submit: function (e) {
          e.preventDefault();
          $("#modal_re_stock_alert .re-stock-alert-form").trigger(
            "ajaxLoadBefore"
          );
          var $this = $(this);
          if (!$this.checkValidation()) {
            ...
            ajax.noCacheCall(
                ...
                if (data.returnCode) {
                  ...
                  if (rCode == "00") {
                    ...
                    dataLayer.push({
                      ...
                      superCategory: $("#reStockAlertForm").attr(
                        "data-super-category-name"
                      ),
                      category:
                        $("#reStockAlertForm").attr("data-category-name"),
                      subcategory: $("#reStockAlertForm").attr(
                      ...
                      salesModelCode: $("#reStockAlertForm").attr(
                        "data-model-salesmodelcode"
                      ),
                      sku: $("#reStockAlertForm").attr("data-sku"),
                      suffix: $("#reStockAlertForm").attr(
              ...
              "post",
              "#modal_re_stock_alert .re-stock-alert-form"
            );
          }
        },
      });
    ...
    init: function () {
      $("body")
        .off("click.restockmembersweek")
        .on("click.restockmembersweek", ".restock-members-week", function (e) {
          e.preventDefault();
          var url = $(this).data("url");
          var dataLayer = {};
  ...
  atcMembersWeek = {
    $nav: $(".navigation").eq(0),
    store: $(".navigation").eq(0).data("store")
      ? $(".navigation").eq(0).data("store")
      : "default",
    form: "#addtocartForm",
    cookieName: "LG5_CartID",
    cartID: null,
    url:
      $("header.navigation") && $("header.navigation").data("cart-check")
        ? $("header.navigation").data("cart-check")
        : "/graphql",
    successMsg: null,
    ...
    init: function () {
      ...
      this.preOrderSuccessMsg = this.$nav
        .find(".cart")
        .data("pre-order-success");
      ...
      this.$nav
        .find(".cart")
        .removeAttr("data-alert-btn")
        .removeAttr("data-alert-success")
        .removeAttr("data-close-btn");
      ...
      $("body")
        .off("click.atcmembersweek")
        .on("click.atcmembersweek", ".atc-members-week", function (e) {
          var isButton = !(
            $(this).attr("role") != "button" ||
            !OBS_LOGIN_FLAG ||
            (OBS_LOGIN_FLAG != "Y" && OBS_LOGIN_FLAG != "S")
          );
          ...
          if (!isButton) {
            ...
            if (OBS_LOGIN_FLAG == "Y") {
              ...
              if (
                !atcMembersWeek.cartID &&
                ($(".navigation").eq(0).data("store") == "sa" ||
                  $(".navigation").eq(0).data("store") == "sa_en")
              ) {
    ...
    setCartNum: function (cnt) {
      if (cnt > 0) {
        if (cnt >= 100) {
          atcMembersWeek.$nav
            .find(
              ".for-mobile .right .cart .count, .for-desktop .right-btm .icons .cart .count"
            )
            .html("99+");
        } else {
          atcMembersWeek.$nav
    ...
    getOrderHistory: function ($obj) {
      if (OBS.authToken) {
        ...
        $.ajax({
          ...
          data: JSON.stringify({
            query:
              "{customerOrders(pageSize: 2 currentPage: 1) {items {id increment_id created_at status billing_address {firstname lastname postcode street city region {region_code} country_id telephone} shipping_address {firstname lastname postcode street city region {region_code} country_id telephone} items {sku qty_ordered price}}}}",
            variables: null,
            operationName: null,
          }),
          dataType: "json",
          ...
          success: function (data) {
            if (data.data.customerOrders == null) {
              ...
              if (len > 0) {
                ...
                if (len2 > 0) {
                  var product =
                    data.data.customerOrders.items[len - 1].items[len2 - 1].sku;
  ...
  $("body").on("click", ".atc-members-week", function () {
    var _this = $(this);
    var price = "";
    var displayName = nvl_def(
      _this.parents(".item").find(".model-name").text()
    );
    ...
    ga360Init = {
      ...
      siteType:
        typeof standardData === "undefined" ? "" : standardData.siteType,
